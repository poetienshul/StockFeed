	...
	try:
		...
		for opt,arg in opts:
			if opt == '--username':
				...
				#querySearch is going to be equal to the company name
		...
		outputFile = codecs.open("data/twitterOutput.csv", "a+", "utf-8")
		...
		def receiveBuffer(tweets):
			for t in tweets:
				try:
					if detect(t.text) == 'en':
						outputFile.write(('%s, %s, %s\n' % (tweetCriteria.querySearch, tweetCriteria.since, t.text.replace(',', ''))))
				except arg:
					print 'undetected something idk honestly'
			outputFile.flush();
